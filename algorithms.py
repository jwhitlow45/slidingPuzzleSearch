...
def a_star_search(initState: str, goalState: str, size: int) -> Tuple[str, int]:
    ...
    while pq.qsize() > 0:
        ...
        for newStateTuple in expand((state, z, moves), size):
            ...
            pq.put((pathCost + 1 + heuristic, pathCost + 1, newState, newZ, newMoves))
        numExpansions += 1
...
def a_star_search_iterative(initState: str, goalState: str, size: int) -> Tuple[str, int]:
    ...
    while pq.qsize() > 0:
        ...
        for newStateTuple in expand((state, z, moves), size):
            ...
            if depth + 1 < depthLimit:
                pq.put((pathCost + 1 + heuristic, pathCost + 1, depth + 1, newState, newZ, newMoves))                
            else:
                tempPQ.put((pathCost + 1 + heuristic, pathCost + 1, 0, newState, newZ, newMoves))
